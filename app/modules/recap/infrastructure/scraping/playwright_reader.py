...
class PlaywrightArticleReader:
    ...
    async def read_article(self, reference: SearchReference) -> Optional[SourceArticle]:
        return await asyncio.to_thread(self._read_article_sync, reference)

    def _read_article_sync(self, reference: SearchReference) -> Optional[SourceArticle]:
        try:
            ...
            logger.error(f"Playwright error reading {reference.url}: {str(e)}")
            return None
        except Exception as e:
            logger.error(f"Unexpected error reading {reference.url}: {str(e)}")
            return None
